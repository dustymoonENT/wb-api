defmodule MotivusWbApiWeb.Ranking.CurrentSeasonRankingControllerTest do
  ...
  describe "index" do
    test "lists all current_season_ranking", %{conn: conn} do
      conn = get(conn, Routes.ranking_current_season_ranking_path(conn, :index))
  ...
  describe "create current_season_ranking" do
    test "renders current_season_ranking when data is valid", %{conn: conn} do
      conn = post(conn, Routes.ranking_current_season_ranking_path(conn, :create), current_season_ranking: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, Routes.ranking_current_season_ranking_path(conn, :show, id))
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.ranking_current_season_ranking_path(conn, :create), current_season_ranking: @invalid_attrs)
  ...
  describe "update current_season_ranking" do
    ...
    test "renders current_season_ranking when data is valid", %{conn: conn, current_season_ranking: %CurrentSeasonRanking{id: id} = current_season_ranking} do
      conn = put(conn, Routes.ranking_current_season_ranking_path(conn, :update, current_season_ranking), current_season_ranking: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, current_season_ranking: current_season_ranking} do
      conn = put(conn, Routes.ranking_current_season_ranking_path(conn, :update, current_season_ranking), current_season_ranking: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete current_season_ranking" do
    ...
    test "deletes chosen current_season_ranking", %{conn: conn, current_season_ranking: current_season_ranking} do
      conn = delete(conn, Routes.ranking_current_season_ranking_path(conn, :delete, current_season_ranking))
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, Routes.ranking_current_season_ranking_path(conn, :show, current_season_ranking))
      end
    end
  end
